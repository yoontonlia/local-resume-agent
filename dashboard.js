...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadAllData() {
        ...
        try {
            ...
            const matchHistory = matchResult.jobMatchHistory || [];
            ...
            // 更新统计卡片
            if (totalAnalyses) totalAnalyses.textContent = analysisHistory.length;
            if (totalMatches) totalMatches.textContent = matchHistory.length;
            if (totalOptimizations) totalOptimizations.textContent = optHistory.length;
            ...
            // 更新最近活动
            updateRecentActivities(analysisHistory, matchHistory, optHistory);
    ...
    // 更新最近活动
    function updateRecentActivities(analysisHistory, matchHistory, optHistory) {
        const activities = [];
                ...
                recentList.innerHTML = activities.slice(0, 10).map(activity => {
                    ...
                    return `
                        <div class="recent-item">
                            <span class="recent-type ${typeClass}">${activity.typeIcon} ${activity.typeName}</span>
                            <span class="recent-name">${escapeHtml(activity.name)}</span>
                            <span class="recent-time">${escapeHtml(activity.time)}</span>
                        </div>
                    `;
                }).join('');
            } else {
